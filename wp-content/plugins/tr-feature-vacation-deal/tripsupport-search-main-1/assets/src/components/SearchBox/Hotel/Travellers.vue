      ...
      <template #data>
        <div class="passenger-menu" v-click-outside="onClickOutside">
          ...
          <div class="ts-error" v-if="getRoomsError">
            ...
            <p>Sorry, Adult count should not be zero.</p>
          </div>
          <div class="ts-error" v-if="getErrorPassengers">
            ...
            <p>
              Sorry, you can only book a maximum of 10 passengers in an online
              booking.
            </p>
          </div>
          <div
            class="passenger-menu-content"
            v-for="index in rooms"
            :key="index"
          >
            <div class="passenger-menu-content-title">Room {{ index }}</div>
            <TravellerItem v-model="value[index - 1]" />
          </div>
          <v-spacer></v-spacer>
          <div class="passenger-menu-done">
            <v-btn
              :disabled="getRoomsError || getErrorPassengers"
...
import TravellerItem from "./TravellerItem.vue";
export default {
  components: {
    MenuDialog,
    TravellerItem,
  ...
  watch: {
    ...
    rooms: {
      handler: function () {
        while (this.model.length < this.rooms) {
          this.model.push({
            adults: 2,
            children: 0,
            childrenAges: [],
        ...
        this.$emit("input", this.model.slice(0, this.rooms));
      },
      deep: true,
    },
  },
  computed: {
    NumberOfPassanger() {
      ...
      for (let item of this.value) {
        total += parseInt(item.adults) + parseInt(item.children);
      }
      return total;
    },
    getErrorPassengers() {
      return this.NumberOfPassanger > 10;
    },
    getRoomsError() {
      debugger;
      return this.model.some((x) => x.adults < 1);

      // return (
      //   (parseInt(this.Children) > 0 && this.rooms > 1) ||
      //
      // );
    },
    getChildAgeError() {
