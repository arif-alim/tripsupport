...
jQuery('.ts-vacation-ul-adults li').click(function () {
    ...
    let rooms = parseInt(jQuery('#ts-vacation-display-rooms').text());
    ...
    if (jQuery('#vacation-from-desktop').val().trim()
        ...
        if (item_vfd?.length) {
            ...
            jQuery.post(ts_config.ajax_url, params_hierarchical_destinations, function (resp) {
                ...
                jQuery(".ts-popup-wrapper").delegate(".ts-vacations-search-popup", "input", function () {
                    let search = jQuery('.ts-vacations-search-popup').val();
                    console.log(search)
                    let cities = jQuery('.ts-popup').find('input[type="checkbox"]');
                    jQuery('.ts-clear').fadeIn(500);
                    for (let i = 0; i < cities.length; i++) {
                        jQuery(cities[i]).parent().fadeIn(500);
                        let chk_name = jQuery(cities[i]).next().find('span').text().toLowerCase();
                        if (!chk_name.startsWith(search.toLowerCase())) {
                            jQuery(cities[i]).parent().fadeOut(500);
                        }
                    }
                    setTimeout(function () {
                        let countries_wrapper = jQuery('.ts-wrapper-chekbox');
                        for (let i = 0; i < countries_wrapper.length; i++) {
                            let chk_cities = jQuery(countries_wrapper[i]).find('.ts-checkbox.ts-checkbox-gray').filter(function () {
                                return jQuery(this).css("display") == "block";
                            });
                            if (jQuery(chk_cities).length == 0) {
                                console.log(jQuery(countries_wrapper[i]).parent().fadeOut(500));
                            }
                        }
                    }, 500)
                ...
                jQuery('.ts-vacation-popup-btn').on('click', function () {
                    ...
                    for (let i = 0; i < popup_cities.length; i++) {
                        let city = jQuery(popup_cities[i]).attr('class');
                        ts_vacation_popup_get_cities.push(city);
                    }
                    jQuery('.ts-popup-wrapper').fadeOut();

                    console.log(ts_vacation_popup_get_cities);
...
jQuery('#ts-btn-vacation').on('click', function () {
    ...
    let display_duration = jQuery('.ts-vacation-display-duration').text();
    let rooms = jQuery('#ts-vacation-display-rooms').text();
    ...
    if (btn_vacation) {
        let storage = [];
        storage.push('v_from', v_from);
        storage.push('cn_from', cn_from);
        storage.push('v_to', v_to);
        storage.push('cn_go', cn_go);
        storage.push('DepartureDate', departureDate);
        storage.push('Durations', duration);
        storage.push('display_duration',display_duration)
        storage.push('AllInclusive', allInclusive);
        storage.push('NumberOfRooms', rooms);
        storage.push('NumberOfAdults', adults);
        storage.push('ChildrenAges', _children);


        localStorage.setItem('vacations', JSON.stringify(storage));
...
jQuery.post(ts_config.ajax_url, params_get_departures, function (resp) {
    ...
    if(performance.navigation.type == 2) {
        let storage = localStorage.getItem('vacations');

        if (storage) {
            flag = false;
            jQuery('#ts-btn-vacation').prop('disabled', false);

            let lst = JSON.parse(storage);
            jQuery("#vacation-from-desktop").typeahead('val', lst[3]).typeahead('open');
            jQuery(".ts-vacation-from-container .tt-suggestion:first-child").trigger('click');

            setTimeout(function () {
                jQuery("#vacation-to-desktop").typeahead('val', lst[7]).typeahead('open');
                jQuery(".ts-vacation-go-to-container .tt-suggestion:first-child").trigger('click');
            }, 3000);


            let t = lst[9].toString().split("-");
            console.log(new Date(t[0], t[1], t[2]))
            console.log(t);
            jQuery('#dp-vd>input[data-toggle="datepicker"]').datepicker('destroy').datepicker({
                trigger: '#dp-vd',
                format: 'yyyy-mm-dd',
                startDate: Date.now(),
            ...
            jQuery('#ts-vacation-select-duration').val(lst[11]);
            jQuery('.ts-vacation-display-duration').text(lst[13]);

            jQuery('#ts-vacation-value-rooms').val(lst[17]);
            jQuery('#ts-vacation-display-rooms').text(lst[17]);
        }
    }
...
jQuery('.ts-flights-one-way-btn').on('click', function () {
    ...
    let isFlexiSearch = jQuery('.ts-flights-one-way-flexsearch').prop('checked');
    let departDate = jQuery('.ts-flights-one-way-dp-departure').datepicker('getDate', true);
    let airlineClass = jQuery('.ts-flights-one-way-airline-class').text();
    ts_flights_one_way_adult = jQuery('.ts-flights-one-way-adult').val();
    ts_flights_one_way_adult_child = jQuery('.ts-flights-one-way-adult-child').val();
    ts_flights_one_way_adult_infant = jQuery('.ts-flights-one-way-adult-infant').val();
